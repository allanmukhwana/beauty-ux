...
final class BeautyUX_A11y_Frontend {

	public $svg_icons = null;

	public function is_toolbar_active() {
		$is_active = 'disable' !== get_option( 'BeautyUX_a11y_toolbar' );

		$is_active = apply_filters( 'BeautyUX_a11y_frontend_is_toolbar_active', $is_active );

		return $is_active;
	}

	public function is_toolbar_button_active( $button_type ) {
		return 'disable' !== get_option( "BeautyUX_a11y_toolbar_button_{$button_type}" );
	}

	public function get_toolbar_button_title( $button_type ) {
		$title = BeautyUX_Accessibility::$instance->settings->get_default_title_text( "BeautyUX_a11y_toolbar_button_{$button_type}_title" );
		return '<span class="BeautyUX-a11y-toolbar-icon">' . $this->get_toolbar_svg( $button_type, $title ) . '</span><span class="BeautyUX-a11y-toolbar-text">' . $title . '</span>';
	}

	public function enqueue_scripts() {
		wp_register_script(
			'BeautyUX-a11y',
			BeautyUX_A11Y_ASSETS_URL . 'js/app.min.js',
			array(
				'jquery',
			),
			'1.0.0',
		...
		wp_register_style(
			'BeautyUX-a11y',
			BeautyUX_A11Y_ASSETS_URL . 'css/style.min.css',
			array(),
			'1.0.0'
		);

		wp_enqueue_script( 'BeautyUX-a11y' );
		wp_enqueue_style( 'BeautyUX-a11y' );

		wp_localize_script(
			'BeautyUX-a11y',
			'BeautyUXA11yOptions',
			array(
				'focusable' => ( 'enable' === get_option( 'BeautyUX_a11y_focusable' ) ),
				'remove_link_target' => ( 'enable' === get_option( 'BeautyUX_a11y_remove_link_target' ) ),
				'add_role_links' => ( 'enable' === get_option( 'BeautyUX_a11y_add_role_links' ) ),
				'enable_save' => ( 'enable' === get_option( 'BeautyUX_a11y_save' ) ),
				'save_expiration' => get_option( 'BeautyUX_a11y_save_expiration' ),
			)
		);
	}

	public function print_skip_to_content_link() {
		$skip_to_content_link = get_option( 'BeautyUX_a11y_skip_to_content_link' );
		if ( 'disable' === $skip_to_content_link ) {
			return;
		}

		$element_id = get_option( 'BeautyUX_a11y_skip_to_content_link_element_id', 'content' );

		?>
		<a id="BeautyUX-a11y-skip-content" class="BeautyUX-skip-link BeautyUX-skip-content" tabindex="1" accesskey="s" href="#<?php echo esc_html( $element_id ); ?>"><?php esc_html_e( 'Skip to content', 'BeautyUX-accessibility' ); ?></a>
		<?php
	}
	...
	public function print_toolbar() {
		...
		$customizer_options = get_option( BeautyUX_A11Y_CUSTOMIZER_OPTIONS );
		...
		$toolbar_title = BeautyUX_Accessibility::$instance->settings->get_default_title_text( 'BeautyUX_a11y_toolbar_title' );
		$toolbar_visibility = get_option( 'BeautyUX_a11y_toolbar' );

		$wrapper_classes = array(
			'BeautyUX-a11y-toolbar-' . $toolbar_position,
		);

		if ( 'enable' !== $toolbar_visibility ) {
			$wrapper_classes[] = 'BeautyUX-a11y-' . $toolbar_visibility;
		}

		$sitemap_link = get_option( 'BeautyUX_a11y_toolbar_button_sitemap_link' );
		$help_link = get_option( 'BeautyUX_a11y_toolbar_button_help_link' );
		$feedback_link = get_option( 'BeautyUX_a11y_toolbar_button_feedback_link' );
		...
		?>
		<nav id="BeautyUX-a11y-toolbar" class="<?php echo esc_attr( implode( ' ', $wrapper_classes ) ); ?>" role="navigation">
			<div class="BeautyUX-a11y-toolbar-toggle">
				<a class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-toolbar-toggle-link" href="javascript:void(0);" title="<?php echo esc_attr( $toolbar_title ); ?>" role="button">
					<span class="BeautyUX-sr-only sr-only"><?php esc_html_e( 'Open toolbar', 'BeautyUX-accessibility' ); ?></span>
			...
			<div class="BeautyUX-a11y-toolbar-overlay">
				<div class="BeautyUX-a11y-toolbar-inner">
					<p class="BeautyUX-a11y-toolbar-title"><?php echo $toolbar_title; ?></p>
					
					<ul class="BeautyUX-a11y-toolbar-items BeautyUX-a11y-tools">
						<?php do_action( 'BeautyUX_a11y_toolbar_before_buttons' ); ?>
						<?php if ( $this->is_toolbar_button_active( 'resize_font' ) ) : ?>
							<li class="BeautyUX-a11y-toolbar-item">
								<a href="#" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-btn-resize-font BeautyUX-a11y-btn-resize-plus" data-action="resize-plus" data-action-group="resize" tabindex="-1" role="button">
									<?php echo $this->get_toolbar_button_title( 'resize_font_add' ); ?>
								</a>
							</li>
							
							<li class="BeautyUX-a11y-toolbar-item">
								<a href="#" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-btn-resize-font BeautyUX-a11y-btn-resize-minus" data-action="resize-minus" data-action-group="resize" tabindex="-1" role="button">
						...
						<?php if ( $this->is_toolbar_button_active( 'grayscale' ) ) : ?>
							<li class="BeautyUX-a11y-toolbar-item">
								<a href="#" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-btn-background-group BeautyUX-a11y-btn-grayscale" data-action="grayscale" data-action-group="schema" tabindex="-1" role="button">
						...
						<?php if ( $this->is_toolbar_button_active( 'high_contrast' ) ) : ?>
							<li class="BeautyUX-a11y-toolbar-item">
								<a href="#" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-btn-background-group BeautyUX-a11y-btn-high-contrast" data-action="high-contrast" data-action-group="schema" tabindex="-1" role="button">
						...
						<?php if ( $this->is_toolbar_button_active( 'negative_contrast' ) ) : ?>
							<li class="BeautyUX-a11y-toolbar-item">
								<a href="#" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-btn-background-group BeautyUX-a11y-btn-negative-contrast" data-action="negative-contrast" data-action-group="schema" tabindex="-1" role="button">
						...
						<?php if ( $this->is_toolbar_button_active( 'light_bg' ) ) : ?>
							<li class="BeautyUX-a11y-toolbar-item">
								<a href="#" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-btn-background-group BeautyUX-a11y-btn-light-background" data-action="light-background" data-action-group="schema" tabindex="-1" role="button">
						...
						<?php if ( $this->is_toolbar_button_active( 'links_underline' ) ) : ?>
							<li class="BeautyUX-a11y-toolbar-item">
								<a href="#" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-btn-links-underline" data-action="links-underline" data-action-group="toggle" tabindex="-1" role="button">
						...
						<?php if ( $this->is_toolbar_button_active( 'readable_font' ) ) : ?>
							<li class="BeautyUX-a11y-toolbar-item">
								<a href="#" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-btn-readable-font" data-action="readable-font" data-action-group="toggle" tabindex="-1" role="button">
									<?php echo $this->get_toolbar_button_title( 'readable_font' ); ?>
								</a>
							</li>
						<?php endif; ?>
						<?php do_action( 'BeautyUX_a11y_toolbar_after_buttons' ); ?>
						<li class="BeautyUX-a11y-toolbar-item">
							<a href="#" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-btn-reset" data-action="reset" tabindex="-1" role="button">
								<span class="BeautyUX-a11y-toolbar-icon"><?php echo $this->get_toolbar_svg( 'reset', esc_html__( 'Reset', 'BeautyUX-accessibility' ) ); ?></span>
								<span class="BeautyUX-a11y-toolbar-text"><?php esc_html_e( 'Reset', 'BeautyUX-accessibility' ); ?></span>
							</a>
						</li>
					</ul>
					<?php if ( $has_custom_links ) : ?>
					<ul class="BeautyUX-a11y-toolbar-items BeautyUX-a11y-links">
						<?php if ( ! empty( $sitemap_link ) ) : ?>
						<li class="BeautyUX-a11y-toolbar-item">
							<a href="<?php echo esc_attr( $sitemap_link ); ?>" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-link-sitemap" tabindex="-1" role="button">
						...
						<li class="BeautyUX-a11y-toolbar-item">
							<a href="<?php echo esc_attr( $help_link ); ?>" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-link-help" tabindex="-1" role="button">
						...
						<li class="BeautyUX-a11y-toolbar-item">
							<a href="<?php echo esc_attr( $feedback_link ); ?>" class="BeautyUX-a11y-toolbar-link BeautyUX-a11y-link-feedback" tabindex="-1" role="button">
